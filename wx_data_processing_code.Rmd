...
pers_burned_rad <- pers_burned_wx_15min %>%
    drop_na() %>%
  filter(datetime >= "2023-12-07 00:00:00" & datetime <= "2024-01-10 00:00:00") %>%
  select("datetime","SWin_Avg", "SWout_Avg")

glimpse(pers_burned_rad)
glimpse(pers_b_aspects)
```
...
### Pers Burned Solar Radiation Data

```{r}
#creating new dataframe for specific dates, to compare to 105E South sensors
pers_burned_rad <- pers_burned_wx_15min %>%
    drop_na() %>%
  filter(datetime >= "2023-12-07 00:00:00" & datetime <= "2024-01-10 00:00:00") %>%
  select("TIMESTAMP", "SWin_Avg", "SWout_Avg")
```
